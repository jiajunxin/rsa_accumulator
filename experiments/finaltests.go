...
func TestNotusSingleThread(setSize, updatedSetSize int) {
	...
	accUpd1 := multiexp.ExpParallel(accMid1, insProd1, setup.N, table, 1, 0)
	accUpd2 := multiexp.ExpParallel(accMid2, insProd2, setup.N, table, 1, 0)
	accUpd3 := multiexp.ExpParallel(accMid3, insProd3, setup.N, table, 1, 0)
	duration = time.Now().UTC().Sub(startingTime)
