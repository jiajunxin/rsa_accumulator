...
func TestMimc() {
	...
	// witness definition
	//outputs := elementFromString("12886436712380113721405259596386800092738845035233065858332878701083870690753")
	assignment := MimcCircuit{Hash: "12886436712380113721405259596386800092738845035233065858332878701083870690753", PreImage: "16130099170765464552823636852555369511329944820189892919423002775646948828469"}
	witness, err := frontend.NewWitness(&assignment, ecc.BN254)
	if err != nil {
		panic(err)
	}
	...
	// groth16: Prove & Verify
	proof, err := groth16.Prove(r1cs, pk, witness) //, backend.IgnoreSolverError()
	if err != nil {
		panic(err)
	}
